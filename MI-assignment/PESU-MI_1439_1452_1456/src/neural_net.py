...
- carries 1 mark
	We have implented Batch Gradient Descent
...
class NN:
	...
	def fit(self, X, Y):
		...
		We have implemented Batch Gradient descent. Here self.batch_size would represent the batch size
		'''
		...
		for e in range(self.epochs):
			...
			for i in range(iterations):
				...
				# Updating parameters through back propagation
	...
	def CM(y_test, y_test_obs, k):
		...
		# print(f"F1 SCORE : {f1}")
		if(ac>0.85):
			print(f"ACCURACY : {k, ac}")

def load_data():
	dataset = '../data/LBW_Dataset_clean.csv'
...
# nn.add(Layer(12, 20, tanh, tanh_prime, learning_rate = 0.05, lr_decay = 0, r_lambda = 0))
...
for i in range(100):
	np.random.seed(i)	
	nn = NN(epochs = 200, batch_size = 2)
	nn.add(Layer(12, 20, tanh, tanh_prime, learning_rate = 0.04, lr_decay = 0, r_lambda = 0))
	nn.add(Layer(20, 16, tanh, tanh_prime, learning_rate = 0.05, lr_decay = 0, r_lambda = 0))
	nn.add(Layer(16, 1, sigmoid, sigmoid_prime, learning_rate = 0.06, lr_decay = 0, r_lambda = 0))
	nn.fit(X_train, Y_train)
	NN.CM(Y_train, nn.predict(X_train), i)
	NN.CM(Y_test, nn.predict(X_test), i)
